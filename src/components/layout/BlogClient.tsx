...
export default function BlogClient({ allPosts, dict, locale }: BlogClientProps) {
    // Filter pinned posts
    const pinnedPosts = allPosts.filter(p => p.featured);
    // Filter regular posts (not featured)
    const regularPosts = allPosts.filter(p => !p.featured);

    const [featuredPost, setFeaturedPost] = useState<Post>(pinnedPosts[0] || allPosts[0]);
    const [isTransitioning, setIsTransitioning] = useState(false);
    const [activeFilter, setActiveFilter] = useState(dict.about.blog.filters[0]);
    const [isSortOpen, setIsSortOpen] = useState(false);
    const [sortBy, setSortBy] = useState(dict.about.blog.sortOptions[0]);

    const filters = dict.about.blog.filters;
                    ...
                    <div className="flex flex-col lg:flex-row justify-between items-center lg:items-start gap-12">
                        ...
                        <div className="w-full lg:w-96 border-8 border-black p-10 bg-white shadow-[8px_8px_0px_#000000]">
                            ...
                            <div className="flex flex-col gap-4">
                                <input 
                                    type="email" 
                                    placeholder={dict.about.blog.newsletterPlaceholder} 
                                ...
                                <Button variant="secondary" size="lg" fullWidth>
                                    <span className="font-lexend font-black uppercase tracking-wider">{dict.about.blog.newsletterButton}</span>
                                </Button>
                            </div>
                        </div>
                    </div>
            ...
            <div className="container mx-auto px-6 py-12">
                    ...
                    <div className="md:col-span-3">
                        <div className="flex flex-col md:flex-row gap-8 items-start">
                                ...
                                <div className="flex flex-col lg:flex-row h-full">
                                    ...
                                    <div className="lg:w-1/2 relative overflow-hidden h-80 lg:h-auto lg:min-h-[500px] border-b-8 lg:border-b-0 lg:border-r-8 border-black bg-gray-100">
                                        <div className="absolute top-4 left-4 z-20">
                                            <Tag variant="primary" shadow>{dict.about.blog.pinnedLabel}</Tag>
                                        </div>
                                    ...
                                    <div className="lg:w-1/2 p-8 md:p-12 flex flex-col justify-center">
                                        ...
                                        <div className="mt-auto pt-8 border-t-4 border-black flex justify-between items-center">
                                            <div className="flex flex-col">
                                                <span className="font-black text-sm text-black uppercase">{featuredPost.date}</span>
                                                <span className="font-black text-xs text-gray-400 uppercase">{featuredPost.readTime}</span>
                                            </div>
                                            <Link href={`/${locale}/blog/${featuredPost.slug}`} className="no-underline">
                                                <Button variant="secondary" size="md">
                                                    <span className="font-black italic uppercase">{dict.about.blog.openStory}</span>
                                                </Button>
                                            </Link>
                                        </div>
                                    </div>
                            ...
                            <div className="md:w-[22%] w-full">
                                <div className="flex flex-col gap-4">
                                    <span className="font-lexend font-black text-xs uppercase italic border-b-4 border-black pb-2 mb-2 text-black">{dict.about.blog.pinnedListTitle}</span>
                ...
                <div className="flex flex-wrap items-center justify-between gap-8 mb-12 border-b-8 border-black pb-10">
                    ...
                    <div className="flex items-center gap-4 relative">
                        <span className="font-lexend font-black uppercase text-xs text-gray-500 italic">{dict.about.blog.sortByLabel}</span>
                        <div className="relative w-44">
                            <button 
                                onClick={() => setIsSortOpen(!isSortOpen)}
                                className="w-full bg-white border-4 border-black h-11 px-4 pr-11 font-black text-xs uppercase flex items-center cursor-pointer relative text-black"
                            >
                                <span>{sortBy}</span>
                            ...
                            {isSortOpen && (
                                <div className="absolute top-full left-0 w-full bg-white border-4 border-black shadow-[6px_6px_0px_#000000] z-50 mt-2">
                                    {dict.about.blog.sortOptions.map((opt: string) => (
                                        <div 
                                            key={opt}
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-10 mb-20">
                    {regularPosts.map((post) => (
                        <Link key={post.slug} href={`/${locale}/blog/${post.slug}`} className="no-underline border-4 border-black bg-white shadow-[8px_8px_0px_#000000] hover:shadow-[10px_10px_0px_#000000] hover:-translate-x-0.5 hover:-translate-y-0.5 transition-all group flex flex-col h-full overflow-hidden">
                ...
                <div className="flex justify-center items-center gap-6 mb-24">
                    ...
                    <Button variant="tertiary" size="md">
                        {dict.about.blog.paginationNext}
                    </Button>
                </div>
